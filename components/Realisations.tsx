...
export function Realisations({ items, hasBefore, spotlight }: { items: Realisation[]; hasBefore: boolean; spotlight: Site['spotlight'] }) {
  const cats = useMemo(() => ['Tous', ...Array.from(new Set(items.map((r) => r.cat)))], [items])
  const [cat, setCat] = useState('Tous')
  const [idx, setIdx] = useState<number | null>(null)
  const list = cat === 'Tous' ? items : items.filter((r) => r.cat === cat)
  ...
  const current = idx === null ? null : list[idx]
    ...
    <section id="realisations" className="py-20 lg:py-28">
      <div className="mx-auto max-w-6xl px-4">
        ...
        <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <Reveal>
            <AvantApres before={hasBefore ? '/images/sdb-avant.jpg' : undefined} after={spotlight.after} title={spotlight.caption} place={spotlight.place} />
          </Reveal>
          <Reveal delay={100}>
            <div className="rounded-3xl bg-white p-7 shadow-card">
              ...
              <h3 className="mt-2 font-display text-3xl font-extrabold uppercase leading-none text-navy">{spotlight.title}</h3>
              <p className="mt-3 leading-relaxed text-ink/70">{spotlight.text}</p>
              <dl className="mt-5 grid grid-cols-3 gap-3 text-center">
                {[[spotlight.surface, 'surface'], [spotlight.days, 'durée'], [spotlight.budget, 'budget TTC']].map(([v, l]) => (
        ...
        <Reveal className="mt-14 flex flex-wrap items-center gap-2">
          {cats.map((c) => (
            <button key={c} onClick={() => { setCat(c); setIdx(null) }} className={`rounded-full border px-4 py-2 text-sm font-semibold transition-colors ${cat === c ? 'border-navy bg-navy text-white' : 'border-line bg-white text-navy hover:border-navy'}`}>
              {c}
            </button>
          ))}
