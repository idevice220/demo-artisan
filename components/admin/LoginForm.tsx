...
export function LoginForm({ demoEmail, demoPassword }: { demoEmail: string; demoPassword: string }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      <button disabled={loading} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[var(--a)] px-5 py-3.5 font-display text-xl font-bold uppercase tracking-wide text-[var(--a-ink)] transition-colors hover:bg-[var(--a-hover)] disabled:opacity-60">
