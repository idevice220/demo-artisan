...
import { ZONES, BUSINESS } from '@/lib/data'
...
export function Zone() {
  ...
  // repère SVG : 1 km = 22 px, centre (0,0) => (300, 200)
  ...
  return (
    <section id="zone" className="bg-white py-20 lg:py-28">
      <div className="mx-auto max-w-6xl px-4">
        <SectionHeading eyebrow="Zone d’intervention" title={<>Le Raincy et tout <span className="text-amber-deep">l’Est parisien.</span></>} subtitle="Basé au Raincy, je couvre une vingtaine de communes. Tapez la vôtre pour connaître mon délai d’intervention." />
          ...
          <Reveal>
            <div className="relative overflow-hidden rounded-3xl border border-line bg-paper shadow-card">
              <svg viewBox="0 0 600 520" className="block h-auto w-full" role="img" aria-label="Carte stylisée des communes couvertes autour du Raincy">
                ...
                {ZONES.map((z) => {
                  const isCenter = z.name === 'Le Raincy'
                  const on = active === z.name
                  return (
                    <g key={z.name} onMouseEnter={() => setHover(z.name)} onMouseLeave={() => setHover(null)} className="cursor-pointer">
                      {isCenter ? (
                        <>
            ...
            <div className="rounded-3xl bg-navy p-6 text-white shadow-lift sm:p-8">
              ...
              <div className="relative mt-4">
                ...
                <datalist id="communes">
                  {ZONES.map((z) => <option key={z.name} value={z.name} />)}
                </datalist>
              </div>
                ...
                {!result && (
                  <div className="flex flex-wrap gap-2">
                    {ZONES.slice(1, 8).map((z) => (
                      <button key={z.name} onClick={() => setQ(z.name)} className="rounded-full border border-white/15 px-3 py-1.5 text-sm text-white/80 hover:border-amber hover:text-amber">{z.name}</button>
                    ))}
                  </div>
                )}
                {result && result.min > 0 && (
                  <div className="rounded-2xl bg-white p-5 text-ink">
                    ...
                    <a href={BUSINESS.tel} className="mt-4 inline-flex items-center gap-2 rounded-full bg-amber px-5 py-2.5 font-display text-lg font-bold uppercase text-ink"><PhoneCall size={17} /> Appeler {BUSINESS.phone}</a>
                  </div>
                )}
                {result && result.min < 0 && (
