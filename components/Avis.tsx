...
import { AVIS, BUSINESS, RATING_SPLIT } from '@/lib/data'
      ...
      <div className="mx-auto max-w-6xl px-4">
          ...
          <Reveal delay={100}>
            <div className="flex flex-col gap-6 rounded-3xl bg-white p-6 shadow-card sm:flex-row sm:items-center">
              <div className="text-center sm:pr-6 sm:text-left">
                <p className="font-display text-7xl font-extrabold leading-none text-navy">{BUSINESS.rating.toLocaleString('fr-FR')}</p>
                ...
                <p className="mt-1 text-sm text-ink/60">{BUSINESS.reviews} avis Google</p>
              </div>
              <ul className="flex-1 space-y-1.5">
                {RATING_SPLIT.map((p, i) => (
        ...
        <div className="relative mt-10">
          <div ref={row} className="no-scrollbar snap-row flex gap-5 overflow-x-auto pb-2">
            {AVIS.map((a) => (
              <article key={a.name} className="snap-item flex w-[85%] shrink-0 flex-col rounded-3xl border border-line bg-white p-6 shadow-card sm:w-[48%] lg:w-[32%]">
                ...
                <div className="mt-5 flex items-center gap-3 border-t border-line pt-4">
                  ...
                  <span className="flex gap-0.5">{Array.from({ length: 5 }).map((_, i) => <Star key={i} size={12} className="fill-amber text-amber" />)}</span>
                </div>
              </article>
            ))}
          </div>
